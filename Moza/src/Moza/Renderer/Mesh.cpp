...
namespace Moza
{
		...
		: m_FilePath(filename)
	{
		...
		//MZ_CORE_INFO("FBX Scene Scale: {0}", factor);
		...
		if (scene->HasMaterials())
		{
			...
			for (uint32_t i = 0; i < scene->mNumMaterials; i++)
			{
				...
				MZ_CORE_TRACE("Material Name = {0}; Index = {1}", aiMaterialName.data, i);
				aiString aiTexPath;
				uint32_t textureCount = aiMaterial->GetTextureCount(aiTextureType_DIFFUSE);
				MZ_CORE_TRACE("  TextureCount = {0}", textureCount);

				aiColor3D aiColor;
				aiMaterial->Get(AI_MATKEY_COLOR_DIFFUSE, aiColor);
				MZ_CORE_TRACE("COLOR = {0}, {1}, {2}", aiColor.r, aiColor.g, aiColor.b);
				...
				if (aiMaterial->GetTexture(aiTextureType_DIFFUSE, 0, &aiTexPath) == AI_SUCCESS)
				{
					...
					if (texture->Loaded())
					{
						...
						MZ_CORE_TRACE("  Texture Path = {0}", texturePath);
						mi->Set("u_AlbedoTexture", m_Textures[i]);
						mi->Set("u_AlbedoTexToggle", 1.0f);
					}
					else
					{
						MZ_CORE_ERROR("Could not load texture: {0}", texturePath);
				...
				for (uint32_t i = 0; i < aiMaterial->mNumProperties; i++)
				{
					...
					MZ_CORE_TRACE("Material Property:");
					MZ_CORE_TRACE("  Name = {0}", prop->mKey.data);

					switch (prop->mSemantic)
					{
					case aiTextureType_NONE:
						MZ_CORE_TRACE("  Semantic = aiTextureType_NONE");
						break;
					case aiTextureType_DIFFUSE:
						MZ_CORE_TRACE("  Semantic = aiTextureType_DIFFUSE");
						break;
					case aiTextureType_SPECULAR:
						MZ_CORE_TRACE("  Semantic = aiTextureType_SPECULAR");
						break;
					case aiTextureType_AMBIENT:
						MZ_CORE_TRACE("  Semantic = aiTextureType_AMBIENT");
						break;
					case aiTextureType_EMISSIVE:
						MZ_CORE_TRACE("  Semantic = aiTextureType_EMISSIVE");
						break;
					case aiTextureType_HEIGHT:
						MZ_CORE_TRACE("  Semantic = aiTextureType_HEIGHT");
						break;
					case aiTextureType_NORMALS:
						MZ_CORE_TRACE("  Semantic = aiTextureType_NORMALS");
						break;
					case aiTextureType_SHININESS:
						MZ_CORE_TRACE("  Semantic = aiTextureType_SHININESS");
						break;
					case aiTextureType_OPACITY:
						MZ_CORE_TRACE("  Semantic = aiTextureType_OPACITY");
						break;
					case aiTextureType_DISPLACEMENT:
						MZ_CORE_TRACE("  Semantic = aiTextureType_DISPLACEMENT");
						break;
					case aiTextureType_LIGHTMAP:
						MZ_CORE_TRACE("  Semantic = aiTextureType_LIGHTMAP");
						break;
					case aiTextureType_REFLECTION:
						MZ_CORE_TRACE("  Semantic = aiTextureType_REFLECTION");
						break;
					case aiTextureType_UNKNOWN:
						MZ_CORE_TRACE("  Semantic = aiTextureType_UNKNOWN");
						break;
					}
					...
					if (prop->mType == aiPTI_String)
					{
						...
						if (key == "$raw.ReflectionFactor|file")
						{
							...
							if (texture->Loaded())
							{
								MZ_CORE_TRACE("  Metalness map path = {0}", texturePath);
				...
				if (aiMaterial->GetTexture(aiTextureType_NORMALS, 0, &aiTexPath) == AI_SUCCESS)
				{
					...
					if (texture->Loaded())
					{
						MZ_CORE_TRACE("  Normal map path = {0}", texturePath);
				...
				if (aiMaterial->GetTexture(aiTextureType_SHININESS, 0, &aiTexPath) == AI_SUCCESS)
				{
					...
					if (texture->Loaded())
					{
						MZ_CORE_TRACE("  Roughness map path = {0}", texturePath);
	...
	void Mesh::TraverseNodes(aiNode* node, const glm::mat4& parentTransform, uint32_t level)
	{
		...
		// MZ_CORE_TRACE("{0} {1}", LevelToSpaces(level), node->mName.C_Str());
