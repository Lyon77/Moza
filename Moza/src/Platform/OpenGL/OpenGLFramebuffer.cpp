...
namespace Moza
{
	...
	void OpenGLFramebuffer::Resize(uint32_t width, uint32_t height)
	{
		...
		{
			MZ_CORE_WARN("Attempted to resize framebuffer to {0}, {1}", width, height);
			return;
		}
		...
		{
			MZ_CORE_ASSERT(m_ColorAttachments.size() <= 4, "Too many Color Attachments!");
		...
		MZ_CORE_ASSERT(glCheckFramebufferStatus(GL_FRAMEBUFFER) == GL_FRAMEBUFFER_COMPLETE, "Framebuffer is incomplete!");
	...
	{
		MZ_CORE_ASSERT(attachmentIndex < m_ColorAttachments.size(), "Attachment Index is out of Range!");
