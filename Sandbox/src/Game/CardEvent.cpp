...
CardEvent::CardEvent(const std::string& path, std::vector<int> points)
{
	for (int i = 0; i < points.size(); i++) {
		m_Cards[i] = Moza::Texture2D::Create(path + "card" + std::to_string(i) + ".png");
...
{
	Moza::Renderer2D::DrawScreenQuad({ 0.0f, 0.0f}, { 4.0f, 3.0f }, m_Cards[index], { 1.0f, 1.0f, 1.0f, 1.0f }, 1.0f);
}
